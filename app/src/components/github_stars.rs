...
pub fn GithubStars() -> impl IntoView {
    let stars = Resource::new(|| (), |_| fetch_github_stars());
    ...
    view! {
        <a
            ...
            aria-label="GitHub"
            class="inline-flex gap-1.5 items-center text-sm transition-colors text-muted-foreground hover:text-foreground"
        >
            <GithubIcon />
            <Suspense fallback=|| {
                view! { <Skeleton class="w-10 h-4" /> }
            }>
                {move || {
                    stars
                        .get()
                        ...
                        .map(|count| {
                            ...
                            view! { <span class="text-xs tabular-nums">{formatted}</span> }
                        })
                }}
            </Suspense>
        </a>
